...
function tm_woocompare_process_button_action() {
	...
	if ( ! wp_verify_nonce( filter_input( INPUT_POST, 'nonce' ), 'tm_woocompare' . $id ) ) {
...
function tm_woocompare_list_render_widget() {
	...
	if ( $products->have_posts() ) {
		...
		while ( $products->have_posts() ) {
			...
			$nonce        = wp_create_nonce( 'tm_woocompare' . $pid );
			...
			$content[]    = '<span class="tm-woocompare-remove" data-id="' . $pid . '" data-nonce="' . $nonce . '">' . $dismiss_icon . '</span>';
	...
	foreach ( $content as $key => $row ) {
		...
		while ( $products->have_posts() ) {
			...
			if ( 1 == $i ) {
				...
				$nonce        = wp_create_nonce( 'tm_woocompare' . $pid );
				$dismiss_icon = apply_filters( 'tm_woocompare_dismiss_icon', '<span class="dashicons dashicons-dismiss"></span>' );
				$html[]       = '<div class="tm-woocompare-remove" data-id="' . $pid . '" data-nonce="' . $nonce . '">' . $dismiss_icon . '</div>';
			}
			$html[] = $row[$pid];
			$html[] = '</' . $tag . '>';
		}
