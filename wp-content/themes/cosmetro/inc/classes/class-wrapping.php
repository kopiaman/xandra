 ...
 */
add_filter('template_include', array('Cosmetro_Wrapping', 'wrap'), 99);
add_filter('cosmetro_wrap_base', 'cosmetro_wrap_base_cpts');
 ...
 */
function cosmetro_template_path()
{
	return Cosmetro_Wrapping::$main_template;
}
 ...
 */
function cosmetro_template_base()
{
	return Cosmetro_Wrapping::$base;
}

if (!class_exists('Cosmetro_Wrapping')) {
	...
	class Cosmetro_Wrapping
	{
		...
		/**
		 * Initialize new instance of Cosmetro_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		{
			$this->templates = apply_filters('cosmetro_wrap_' . $this->slug, $this->templates);
		 ...
		 * and $base as static variables in Cosmetro_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function cosmetro_wrap_base_cpts($templates)
{
