 ...
 * @package Cosmetro
 */
class Cosmetro_About_Author_Widget extends Cherry_Abstract_Widget
{
	...
	public function __construct()
	{
		...
		$this->widget_cssclass = 'cosmetro widget-about-author';
		$this->widget_description = esc_html__('Display an information about selected user.', 'cosmetro');
		$this->widget_id = 'cosmetro_widget_about_author';
		$this->widget_name = esc_html__('About Author', 'cosmetro');
		$this->settings = array(
			'title'       => array(
				'type'  => 'text',
				'value' => esc_html__('About Author', 'cosmetro'),
			...
			'user_id'     => array(
				...
				'options'          => false,
				'label'            => esc_html__('Select user to show', 'cosmetro'),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => esc_html__('Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'cosmetro'),
			),
			'avatar_img'  => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => esc_html__('Select image', 'cosmetro'),
				'label'              => esc_html__('Custom avatar image (override default user avatar)', 'cosmetro'),
			),
			'link'        => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__('Link (leave empty to hide)', 'cosmetro'),
			),
			'link_text'   => array(
				'type'  => 'text',
				'value' => esc_html__('Read More', 'cosmetro'),
				'label' => esc_html__('Link label', 'cosmetro'),
			),
		);
	...
	public function get_users_list()
	{
		...
		$result = array('0' => esc_html__('Select a user', 'cosmetro'));
...
add_action('widgets_init', 'cosmetro_register_about_author_widgets');
function cosmetro_register_about_author_widgets()
{
	register_widget('Cosmetro_About_Author_Widget');
}
