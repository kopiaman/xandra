...
class Cosmetro_Custom_Posts_Widget extends Cherry_Abstract_Widget
{
	...
	{
		$this->widget_name = esc_html__('Custom Posts', 'cosmetro');
		$this->widget_description = esc_html__('Display custom posts your site.', 'cosmetro');
		$this->widget_id = apply_filters('cosmetro_custom_posts_widget_ID', 'widget-custom-postson');
		$this->widget_cssclass = apply_filters('cosmetro_custom_posts_widget_cssclass', 'widget-custom-postson');

		$this->settings = array(
			'title'          => array(
				'type'  => 'text',
				'value' => esc_html__('Custom Posts', 'cosmetro'),
			...
			'terms_type'     => array(
				...
				'options' => array(
					...
					'post_format'   => array(
						'label' => esc_html__('Post Format', 'cosmetro'),
			...
			'post_format'    => array(
				...
				'options'          => false,
				'label'            => esc_html__('Select post format', 'cosmetro'),
				'multiple'         => true,
				'placeholder'      => esc_html__('Select post format', 'cosmetro'),
			...
			'posts_per_page' => array(
				...
				'max_value' => 50,
				'min_value' => 0,
				'label'     => esc_html__('Posts count ( Set 0 to show all. )', 'cosmetro'),
			),
			'post_offset'    => array(
				'type'       => 'stepper',
				'value'      => '0',
			...
			'excerpt_length' => array(
				...
				'min_value'  => '0',
				'step_value' => '1',
				'label'      => esc_html__('Excerpt words length ( Set 0 to hide excerpt. )', 'cosmetro'),
			),
			'mate_data'      => array(
				'type'    => 'checkbox',
				'value'   => array(
				...
				'options' => array(
					'date'          => esc_html__('Date', 'cosmetro'),
					'author'        => esc_html__('Author', 'cosmetro'),
					'comment_count' => esc_html__('Comment count', 'cosmetro'),
					'category'      => esc_html__('Category', 'cosmetro'),
					'post_tag'      => esc_html__('Tag', 'cosmetro'),
					'more_button'   => esc_html__('More Button', 'cosmetro'),
				),
				'label'   => esc_html__('Display post meta data', 'cosmetro'),
			),
			'button_text'    => array(
				'type'  => 'text',
				'value' => 'Read More',
				'label' => esc_html__('Post read more button label', 'cosmetro'),
			),
		);
	...
	{
		return ('0' !== $length) ? wp_trim_words($text, $length, esc_html__(' ...', 'cosmetro')) : '';
	}
	...
	{
		return ($visible === 'true') ? '<span class="post_comments"><i class="material-icons">chat_bubble_outline</i>' . $count . '</span>' : '';
	}
	 ...
	 * @return string
	 */
	public function get_post_image($post, $image_size = 'cosmetro-thumb-m', $image_mobile_size = 'cosmetro-thumb-s')
	{
		...
		if ($post_thumbnail_id) {
			...
			$placeholder_attr = apply_filters('cosmetro_custom_posts_placeholder_default_args',
				array(
					'width'      => $size['width'],
					'height'     => $size['height'],
					'background' => get_theme_mod('regular_accent_color_1', cosmetro_theme()->customizer->get_default('regular_accent_color_1')),
					'foreground' => get_theme_mod('regular_accent_color_2', cosmetro_theme()->customizer->get_default('regular_accent_color_2')),
	...
	public function get_button($text = '', $visible = 'true')
	{
		if ($visible === 'true') {
			$permalink = $this->get_post_permalink();
			$icon = apply_filters('cosmetro_custom_posts_read_more_icon', ''); //<span class="btn__icon"></span>
	...
	public function widget($args, $instance)
	{
		...
		if ($posts) {
			...
			$image_size = apply_filters('cosmetro_playlist_thumbnail_image_size', 'cosmetro-thumb-536-449');
			$image_mobile_size = apply_filters('cosmetro_playlist_thumbnail_size_mobile', 'cosmetro-thumb-149-126');
...
add_action('widgets_init', 'cosmetro_register_custom_posts_widget');
function cosmetro_register_custom_posts_widget()
{
	register_widget('Cosmetro_Custom_Posts_Widget');
}