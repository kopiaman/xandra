 ...
 * @return void
 */
function cosmetro_blog_content($length = 5)
{
	...
	$blog_content = get_theme_mod('blog_posts_content', cosmetro_theme()->customizer->get_default('blog_posts_content'));
 ...
 * @return void
 */
function cosmetro_post_excerpt($args = array())
{
 ...
 * @return void
 */
function cosmetro_post_content()
{

	the_content(sprintf(
	/* translators: %s: Name of current post. */
		wp_kses(esc_html__('Continue reading %s <span class="meta-nav">&rarr;</span>', 'cosmetro'), array('span' => array('class' => array()))),
...
function cosmetro_post_thumbnail_size_class()
{
	...
	echo 'post-thumbnail--' . $size . $no_thumbnail;
}
 ...
 * @return void
 */
function cosmetro_post_thumbnail($linked = false, $sizes = array())
{
	...
	$sizes = wp_parse_args($sizes, array(
		'small'     => 'cosmetro-thumb-536-449',
		'fullwidth' => 'cosmetro-thumb-post-thumbnail-large',
	));

	$linked_format = apply_filters(
		'cosmetro_linked_post_thumbnail_format',
		'<a href="%2$s" class="post-thumbnail__link %3$s">%1$s</a>'
	);

	$single_format = apply_filters(
		'cosmetro_single_post_thumbnail_format',
		'%1$s'
	);
	...
	$size = apply_filters('cosmetro_post_thumbail_size', false);
 ...
 * @return void
 */
function cosmetro_read_more()
{
	$button_text = get_theme_mod('blog_read_more_text', cosmetro_theme()->customizer->get_default('blog_read_more_text'));

	if (!$button_text) {
		return;
	}

	$format = apply_filters('cosmetro_read_more_button_format', '<a href="%2$s" class="btn"><span class="btn__text">%1$s</span><span class="btn__icon"></span></a>');
